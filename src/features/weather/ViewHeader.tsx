...
export default function ViewHeader(props: {location?: Location, isDefaultLocation?: boolean}){
    ...
    return  <div className="hsm:self-start flex w-full gap-1 items-center">
        ...
        <span className="text-zinc-500 select-none hidden sm:inline">{`${new Date().toLocaleDateString()}`}</span>
        <div className="w-full"></div>
        {(!props.isDefaultLocation && !props.location?.isAutoDetected) ? <Button size={"lg"} className="z-20 rounded-xl hidden sm:block border-2 border-border hover:bg-accent hover:text-accent-foreground" 
            variant={"ghost"}
            onClick={()=>{
