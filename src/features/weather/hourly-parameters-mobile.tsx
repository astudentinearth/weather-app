...
export function MobileHourlyParameters(){
    ...
    return <Tabs defaultValue="wind" className="bg-background border border-border m-3 rounded-xl sm:hidden">
        <TabsList className="w-full flex p-1 gap-1 bg-background rounded-2xl">
            <TabsTrigger className="w-full data-[state=active]:bg-secondary rounded-md text-lg py-3" value="wind">{t("ui.wind_tab")}</TabsTrigger>
            <TabsTrigger className="w-full data-[state=active]:bg-secondary rounded-md text-lg py-3" value="precipitation">{t("ui.precipitation_tab")}</TabsTrigger>
        </TabsList>
