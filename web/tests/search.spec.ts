...
test.beforeEach(async ({ context }) => {
    context.route(/forecast.*daily/, async (route) => {
        const str = (
            await fs.readFile("./web/tests/data/mock-daily.json")
        ).toString();
    ...
    context.route(/forecast.*hourly/, async (route) => {
        const str = (
            await fs.readFile("./web/tests/data/mock-hourly.json")
        ).toString();
        const json = JSON.parse(str);
        route.fulfill({ json });
    });
    context.route(/forecast.*current/, async (route) => {
        const str = (
            await fs.readFile("./web/tests/data/mock-current.json")
        ).toString();
        const json = JSON.parse(str);
        route.fulfill({ json });
    });
...
test.describe("desktop search tests", () => {
    test("ctrl+k should trigger search on desktop", async ({ page }) => {
        ...
        expect(dialog, "dialog should be open").toHaveAttribute(
            "data-state",
            "open"
        );
...
test.describe("mobile search tests", () => {
    ...
    test("ctrl+k should trigger search on mobile", async ({ page }) => {
        ...
        expect(popover, "dialog should be open").toHaveAttribute(
            "data-state",
            "open"
        );
        expect(input, "input should be focused").toBeFocused();
