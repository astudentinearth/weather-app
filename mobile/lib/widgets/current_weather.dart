...
class _CurrentWeatherWidgetState extends State<CurrentWeatherWidget> {
  ...
  void didChangeDependencies() {
    ...
    if (!didFetch) {
      ...
      Provider.of<WeatherViewModel>(context, listen: false).changeUnits(units);
  ...
  Widget build(BuildContext context) {
    return Consumer<WeatherViewModel>(
      builder: (context, weather, child) {
        return Consumer<UserPrefsViewModel>(
          builder: (context, prefs, child) {
            weather.changeUnits(prefs.getUnits());
            return Container(
                        ...
                        child: Column(
                          children: [
                            Text(
                              ...
                              style:
                                  defaultText.withSize(64).copyWith(height: 1),
                            ),
                            Text(
                                "${weather.current.minTemperature.round()}${weather.units.temperature.withSign()} / ${weather.current.maxTemperature.round()}${weather.units.temperature.withSign()}",
